...
- Standard Google OAuth2 flow via Chrome Identity API — user grants Drive file access (limited to files created by TabNest)
- TabNest creates a folder: **"TabNest Backups"** in the user's Google Drive
- A single JSON file per workspace is stored in this folder

### 9.2 Sync Behavior

| Setting | Detail |
|---|---|
| Auto-sync frequency | Per-device setting, default every 15 minutes; driven by `chrome.alarms` while the browser is running |
| Conflict resolution | Workspaces and trash union-merge by entity id, with deletions propagated as trash tombstones; where both sides hold the same entity, the newer `updated_at` wins its scalar fields. Settings are last-write-wins by `last_modified_at`. The local workspaces are snapshotted (last 3 generations) before any merge is applied |
...
- OAuth scope requested: `https://www.googleapis.com/auth/drive.appdata` — TabNest reads and writes only its own hidden application-data folder (`appDataFolder`). It has no access to the user's own Drive files, and the sync file is not visible in the user's Drive UI
...
| Storage limit | `chrome.storage.local` limit is 10MB by default; Drive sync removes effective limit |
