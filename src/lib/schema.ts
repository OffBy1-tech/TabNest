...
export const CategorySchema = z.object({
  ...
  /**
   * Last content-relevant edit — rename, color/emoji, reorder, restore, or a
   * change to `collapsed` (which is a synced All-view visibility filter, not
   * device-local viewport state). Any mutation touching a field above must
   * bump this or mergeCategories discards it on every other device. Optional
   * so pre-existing data parses. Also feeds tombstone suppression in merge.ts
   * — an empty category restored from trash survives the merge only through
   * this.
   */
...
export const WorkspaceSchema = z.object({
   ...
   * data parses. Feeds mergeWorkspaces (via newerFirst) and tombstone
   * suppression in merge.ts — an empty workspace restored from trash survives
...
export const TrashItemSchema = z.object({
   ...
   * need deletion to propagate through mergeSyncedState instead of
...
export const StorageSchemaZod = z.object({
   ...
   * Lazy factory so it can reference DEFAULT_LOCAL_SETTINGS (declared below)
   * instead of duplicating its literals.
   */
  local_settings: LocalSettingsSchema.optional().default(() => ({ ...DEFAULT_LOCAL_SETTINGS })),
