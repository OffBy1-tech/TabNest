...
async function readDocSummary(): Promise<{ workspaces: Workspace[]; syncState: SyncState }> {
  try {
...
export function PopupApp(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    async function init(): Promise<void> {
      ...
      const [tabInfo, docInfo, popupState] = await Promise.all([
        readActiveTab(),
        readDocSummary(),
        readPopupState(),
      ]);
      ...
      try {
        const response = await sendExtensionMessage<{ saved: number; group_id: string }>({
          ...
          payload: {
            ...
            // Append into the chosen group; omitted for "New group…" so the
        ...
        const next: RecentGroup = {
          groupId: response.data?.group_id ?? target.groupId,
