...
describe('WindowSection', () => {
  ...
  // content-visibility implies contain:paint, which clips the absolutely-
  // positioned SavePopover to the 40px row — containment must lift while open.
  it('drops row content-visibility containment while its save popover is open', () => {
    renderSection()
    const saveButton = screen.getByRole('button', { name: 'Save First' })
    const row = saveButton.closest('[draggable="true"]') as HTMLElement
