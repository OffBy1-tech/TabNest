...
describe('mergeImportedData', () => {
  ...
  it('carries standalone category notes through a matched-category merge (with fresh ids)', () => {
    const incomingNote = { id: crypto.randomUUID(), content: 'keep me', created_at: 1, updated_at: 1 };
    const currentNote = { id: crypto.randomUUID(), content: 'already here', created_at: 1, updated_at: 1 };
    const currentCat = { ...category('Work', [group('A')]), notes: [currentNote] };
    const incomingCat = { ...category('work', [group('B')]), notes: [incomingNote] };
    const current = storage([workspace('Personal', [currentCat])]);
    const incoming = storage([workspace('Personal', [incomingCat])]);
    const result = mergeImportedData(current, incoming);
    const merged = result.workspaces[0]!.categories[0]!;
    expect(merged.notes.map((n) => n.content).sort()).toEqual(['already here', 'keep me']);
    const importedNote = merged.notes.find((n) => n.content === 'keep me')!;
    expect(importedNote.id).not.toBe(incomingNote.id);
  });
  it('regenerates standalone note ids in unmatched category and workspace branches', () => {
    const noteA = { id: crypto.randomUUID(), content: 'a', created_at: 1, updated_at: 1 };
    const noteB = { id: crypto.randomUUID(), content: 'b', created_at: 1, updated_at: 1 };
    const current = storage([workspace('Personal', [category('Reading', [])])]);
    const incoming = storage([
      workspace('Personal', [{ ...category('New Cat', []), notes: [noteA] }]),
      workspace('Brand New WS', [{ ...category('C', []), notes: [noteB] }]),
    ]);
    const result = mergeImportedData(current, incoming);
    const newCat = result.workspaces[0]!.categories.find((c) => c.name === 'New Cat')!;
    expect(newCat.notes[0]!.content).toBe('a');
    expect(newCat.notes[0]!.id).not.toBe(noteA.id);
    const newWs = result.workspaces.find((w) => w.name === 'Brand New WS')!;
    expect(newWs.categories[0]!.notes[0]!.id).not.toBe(noteB.id);
  });
});
